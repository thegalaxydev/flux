...
static COMPONENTS: RwLock<Vec<ComponentSerde>> = RwLock::new(Vec::new());
...
/// component type at plugin install.
pub fn register_component(
    name: &'static str,
    serialize: fn(&World, InstanceId) -> Option<Json>,
    deserialize: fn(&mut World, InstanceId, &Json),
) {
    COMPONENTS.write().unwrap().push(ComponentSerde {
        name,
        serialize,
        deserialize,
    });
...
pub(crate) fn save_components(world: &World, id: InstanceId) -> IndexMap<String, Json> {
    ...
    for c in COMPONENTS.read().unwrap().iter() {
...
pub(crate) fn load_components(world: &mut World, id: InstanceId, comps: &IndexMap<String, Json>) {
    let regs = COMPONENTS.read().unwrap();
