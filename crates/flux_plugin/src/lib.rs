...
//! Names resolve to platform library files searched next to the running
//! executable, then in the project root (and its `plugins/` subfolder).
...
/// Resolve a manifest plugin name to a library file: next to the running
/// executable first (cargo puts workspace dylibs there), then the project
/// root and its `plugins/` folder. Absolute/relative paths pass through.
pub fn resolve_plugin(root: &Path, name: &str) -> Option<PathBuf> {
    ...
    candidates.push(root.join("plugins").join(&file));
    candidates.push(root.join(&file));
    candidates.push(PathBuf::from(name)); // explicit path in the manifest
    ...
    for name in &missing {
        let Some(path) = resolve_plugin(root, name) else {
            return Ensure::Error(format!(
                "plugin '{name}' not found (looked next to the executable and in {})",
                root.display()
            ));
        };
